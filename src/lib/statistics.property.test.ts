    ...
    fc.assert(
      fc.property(
        ...
        (offset, slope, intercept) => {
          const xValues = [-2, -1, 0, 1, 2].map(
            (delta) => offset + delta,
          )
          const yValues = [-2, -1, 0, 1, 2].map(
            (delta) => intercept + slope * delta,
          )
          const fit = fitRegression(xValues, yValues, "linear")

          expect(fit.ok).toBe(true)
          expect(fit.predict(offset + 3)).toBeCloseTo(
            intercept + 3 * slope,
            7,
          )
    ...
    fc.assert(
      fc.property(
        ...
        (values) => {
          ...
          expect(quantile(result.sorted, 0.25)).toBeLessThanOrEqual(
            quantile(result.sorted, 0.5),
          )
          expect(quantile(result.sorted, 0.5)).toBeLessThanOrEqual(
            quantile(result.sorted, 0.75),
          )
