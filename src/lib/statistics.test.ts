...
describe("statistics utilities", () => {
  ...
  it("uses stable arithmetic for cancellation and extreme quantiles", () => {
    ...
    const overflowThenCancellation = calculateStatistics([
      1e308,
      1e308,
      -1e308,
    ])
  ...
  it("computes correlation without overflowing intermediate squares", () => {
    expect(
      correlation([1e200, 2e200, 3e200], [2e200, 4e200, 6e200]),
    ).toBeCloseTo(1)
    expect(
      covariance([1e200, 2e200, 3e200], [1e-200, 2e-200, 3e-200]),
    ).toBeCloseTo(1)
  })
  ...
  it("fits translated and extreme-scale polynomial data without overflow", () => {
    ...
    const translated = fitRegression(
      translatedX,
      [3, 5, 7, 9],
      "linear",
    )
    const scaledLinear = fitRegression(
      [1e200, 2e200, 3e200, 4e200],
      [2, 4, 6, 8],
      "linear",
    )
    const scaledQuadratic = fitRegression(
      [1e100, 2e100, 3e100, 4e100],
      [1, 4, 9, 16],
      "quadratic",
    )
