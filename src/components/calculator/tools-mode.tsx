      ...
      <div className="mirai-tools-grid grid gap-4">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2 rounded-lg bg-muted p-3">
              <ResultRow
                ...
                value={formatNumber(percentResults.portion)}
              />
              <ResultRow
                label="Increased by"
                value={formatNumber(percentResults.increased)}
              />
              <ResultRow
                label="Decreased by"
                value={formatNumber(percentResults.decreased)}
              />
              <ResultRow
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Simplify, scale, and check equivalence.
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-3 gap-3">
              <NumberField
                label="a"
                value={ratioLeft}
                onChange={setRatioLeft}
              />
              <NumberField
                label="b"
                value={ratioRight}
                onChange={setRatioRight}
              />
              <NumberField
                label="Scale"
                value={ratioScale}
                onChange={setRatioScale}
              />
            </div>
            <div className="space-y-2 rounded-lg bg-muted p-3">
              <ResultRow
              ...
              <ResultRow
                label="As decimal"
                value={formatNumber(ratioResults.decimal)}
              />
              <ResultRow
                label="gcd · lcm"
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Two points in, distance and slope out.
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2 rounded-lg bg-muted p-3">
              <ResultRow
                label="Distance"
                value={formatNumber(coordinateResults.distance)}
              />
              <ResultRow
                label="Midpoint"
        ...
        <Card className="mirai-tools-wide border-primary/20 bg-primary/5">
          <CardContent className="py-4">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              These helpers perform arithmetic only. They do not factor, solve,
              or explain a question, keeping the workflow appropriate for
              timed practice.
            </p>
          </CardContent>
        </Card>
