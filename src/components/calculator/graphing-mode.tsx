...
import { CalculatorEngine, type AngleMode } from "@/lib/calculator-engine"
import {
...
import { fitRegression } from "@/lib/statistics"
...
type ExplicitYExpression = Extract<
...
export function GraphingMode({
  ...
  const formatNumber = useCallback(
    (value: number) =>
      formatValue(
        Number.isFinite(value) ? Number(value.toPrecision(7)) : value,
      ),
    [formatValue],
  )
  ...
  const marks = useMemo<GraphMark[]>(() => {
    ...
    for (const item of visibleFunctions) {
      try {
        ...
        for (const point of findExtrema(
          item.expression.evaluate,
          view.xmin,
          view.xmax,
        )) {
        ...
        if (expression.kind === "explicit") {
          ...
          for (let pixel = 0; pixel <= count; pixel += 1) {
            ...
            try {
              ...
              const canvasX =
                expression.axis === "y" ? pixel : xToCanvas(output)
              const canvasY =
                expression.axis === "y" ? yToCanvas(output) : pixel
              const valid =
          ...
          for (let column = 0; column < columns; column += 1) {
            for (let rowIndex = 0; rowIndex < rowsCount; rowIndex += 1) {
              const x0 =
                view.xmin +
                (column / columns) * (view.xmax - view.xmin)
              const x1 =
                view.xmin +
                ((column + 1) / columns) * (view.xmax - view.xmin)
              const y0 =
                view.ymax -
                (rowIndex / rowsCount) * (view.ymax - view.ymin)
              const y1 =
                view.ymax -
                ((rowIndex + 1) / rowsCount) * (view.ymax - view.ymin)
              try {
                const values = [
            ...
            currentRows.map((current) => {
              ...
              return {
                ...
                slider: {
                  ...current.slider,
                  value:
                    next > current.slider.max ? current.slider.min : next,
                },
              }
            }),
          )
  ...
  const graphCoordinates = (clientX: number, clientY: number): GraphPoint => {
    ...
    return {
      x:
        view.xmin +
        ((clientX - bounds.left) / bounds.width) * (view.xmax - view.xmin),
      y:
        view.ymax -
        ((clientY - bounds.top) / bounds.height) * (view.ymax - view.ymin),
    }
  }
            ...
            {compiled.map(({ row, expression }) => (
              <div key={row.id} className="flex gap-2.5 px-3 py-3">
                <Button
                  ...
                  aria-label={
                    row.visible ? "Hide expression" : "Show expression"
                  }
                  ...
                  {row.slider && (
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        aria-label={
                          row.slider.playing ? "Stop slider" : "Play slider"
                        }
                ...
                {table.map((row, index) => (
                  <div className="contents" key={index}>
                    <Input
                      value={row.x}
                      onChange={(event) =>
                        updateCell(index, "x", event.target.value)
                      }
                    ...
                    <Input
                      value={row.y}
                      onChange={(event) =>
                        updateCell(index, "y", event.target.value)
                      }
          ...
          <div className="space-y-1">
            {marks.slice(0, 12).map((mark, index) => (
              <Button
                ...
                onClick={() =>
                  setTrace({ x: mark.x, y: mark.y, color: mark.color })
                }
            ...
            {marks.length === 0 && !trace && (
              <p className="py-2 text-xs text-muted-foreground">
                Click the graph to trace a curve. Zeros, extrema, and
                intersections appear here.
              </p>
            )}
          </div>
        </div>
