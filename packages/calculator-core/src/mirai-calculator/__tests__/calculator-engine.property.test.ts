...
function expectRelativelyClose(actual: number, expected: number, tolerance = 1e-10) {
...
describe("CalculatorEngine property checks", () => {
    ...
    fc.assert(
      fc.property(
        ...
        (left, right) => {
          ...
          const difference = evaluateExpression(`(${left})−(${right})`) as number
  ...
  it("preserves absolute-value and square-root invariants", () => {
    fc.assert(
      fc.property(finiteNumber, (value) => {
        ...
        expectRelativelyClose(evaluateExpression(`sqrt((${value})²)`) as number, Math.abs(value))
      }),
  ...
  it("matches an independent Euclidean GCD implementation", () => {
    fc.assert(
      fc.property(
        ...
        fc.integer({ min: -1_000_000, max: 1_000_000 }),
        (left, right) => {
          expect(greatestCommonDivisor(left, right)).toBe(referenceGcd(left, right))
        }
      ),
      ...
      fc.property(
        ...
        (n, candidate) => {
          ...
          expectRelativelyClose(
            factorial(r) * (evaluateExpression(`ncr(${n},${r})`) as number),
  ...
  it("preserves logarithm, exponential, and root identities", () => {
    fc.assert(
      fc.property(
        ...
        (value) => {
          expectRelativelyClose(evaluateExpression(`ln(exp(${value}))`) as number, value, 2e-12)
          expectRelativelyClose(evaluateExpression(`cbrt((${value})³)`) as number, value, 2e-12)
        }
      ),
    ...
    fc.assert(
      fc.property(
        ...
        (value) => {
          ...
          expectRelativelyClose(degrees.evaluate(`sin(asin(${value}))`) as number, value, 2e-12)
          expectRelativelyClose(radians.evaluate(`cos(acos(${value}))`) as number, value, 2e-12)
        }
      ),
  ...
  it("keeps Euclidean modulo in range and reconstructs integer dividends", () => {
    fc.assert(
      fc.property(
        ...
        (dividend, divisor) => {
          const remainder = evaluateExpression(`mod(${dividend},${divisor})`) as number
  ...
  it("makes decimal rounding idempotent across generated precisions", () => {
    fc.assert(
      fc.property(finiteNumber, fc.integer({ min: -6, max: 6 }), (value, precision) => {
        const once = evaluateExpression(`round(${value},${precision})`) as number
        const twice = evaluateExpression(`round(${once},${precision})`) as number
